...
if (COHORT_FILE && existsSync(COHORT_FILE)) {
  ...
  console.log(
    `[fake-social-api] loaded cohort overlay from ${COHORT_FILE} (${Object.keys(cohort).length} records)`,
  )
}
...
// Returns the raw social/business-presence profile for an account, the way a
// real social-data provider's API would. The adapter maps it to canonical.
app.post("/v1/profiles/lookup", (req, res) => {
  ...
  if (!record || record._comment) {
    return res.status(404).json({ error: "profile_not_found", ic })
  }
  res.json({
    ic,
    ...
    engagement90d: record.engagement90d,
    posting12m: record.posting12m,
    reviews: record.reviews,
    sentiment90d: record.sentiment90d,
    flags24m: record.flags24m,
  })
})

app.listen(PORT, () => {
  console.log(`[fake-social-api] listening on :${PORT} (${Object.keys(APPLICANTS).length} applicants)`)
})
