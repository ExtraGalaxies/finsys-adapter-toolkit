...
a partner ships when building a real adapter against a major telco, a payment gateway,
a POS-terminal network, etc.
...
- **Realistic shape.** `fetch(identity)` returns a real HTTP payload
  the adapter then transforms — the same flow a a major telco or a POS-terminal network adapter
  follows.
